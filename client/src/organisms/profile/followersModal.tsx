...
const FollowersModal = (props: Props) => {
  ...
  const { data: getUserFollowersData, error: getUserFollowersError } = useQuery<
    GetUserFollowersQuery,
    GetUserFollowersQueryVariables
  >(GetUserFollowersDocument, {
  ...
  useEffect(() => {
    if (getUserFollowersError) {
      toast.error(getUserFollowersError.message);
    }
      ...
      {isModal && (
        ...
        >
          ...
          >
            ...
            {getUserFollowersData?.getUserFollowers.map((follow) => (
              <Link
